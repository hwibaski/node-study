...
export class AuthService {
   ...
   *
   * 4) signToken
   *    - (3)에서 필요한 액세스 토큰과 리프레쉬 토큰을 사인하는 로직
   *
   ...
   *
   * 3) 아무나 접근할 수 없는 정보를 요청할 때는 accessToken을 Header에 추가해서 요청과 함께 보낸다.
   *    예) {authorization: 'Bearer {token}'}
   *
   * 4) 토큰과 요청을 함께 받은 서버는 토큰 검증을 통해 현재 요청을 보낸 사용자가 누구인지 알 수 있다.
   *    예를 들어서 현재 로그인한 사용자가 작성한 포스트만 가져오려면 토큰의 sub 값에 입력되어 있는 사용자의 포스트만 따로 필터링
   ...
   *    refresh 토큰을 새로 발급 받을 수 있는 /auth/token/refresh가 필요(기획에 따라 재로그인 시킬 수도 있음)
   *
